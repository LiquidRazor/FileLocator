...
FileLocator is a PHP 8.3+ library that loads discovery configuration through `liquidrazor/config-loader`, validates it, and lazily yields matching PHP files from configured roots.
...
- [Configuration system](02_architecture/config_system.md): ConfigLoader integration, config sources, schema, merge behavior, normalization, and validation.
- [File locator API](02_architecture/file_locator.md): `FileLocator` usage, traversal rules, filtering, and failure modes.
- [Config loading integration](02_architecture/yaml_strategy.md): How FileLocator delegates config loading to ConfigLoader.
